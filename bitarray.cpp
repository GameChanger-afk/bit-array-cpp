    ...
    : data(nullptr), num_bits(0), capacity(0)
{
    ...
    if (b.num_bits > 0)
    {
        std::memcpy(data, b.data, get_words_needed(b.num_bits) * sizeof(unsigned long));
    }
}
...
void BitArray::allocate(int bits)
{
    ...
    int words = get_words_needed(bits);
...
int BitArray::get_words_needed(int bits) const
{
...
void BitArray::resize(int num_bits, bool value)
{
    ...
    int old_words = get_words_needed(old_size);

    allocate(num_bits);

    if (old_size > 0)
    {
        int words_to_copy = std::min(old_words, get_words_needed(num_bits));
        ...
        if (num_bits < old_size)
        {
            ...
            if (excess > 0)
            {
                ...
                data[get_words_needed(num_bits) - 1] &= mask;
            }
        }
    }
...
BitArray& BitArray::operator&=(const BitArray& b)
{
    ...
    int words = get_words_needed(num_bits);
...
BitArray& BitArray::operator<<=(int n)
{
    ...
    if (n >= num_bits)
    {
        std::memset(data, 0, get_words_needed(num_bits) * sizeof(unsigned long));
        return *this;
    }
    if (n == 0)
    {
    ...
    int total_words = get_words_needed(num_bits);
...
{
    std::memset(data, 0xFF, get_words_needed(num_bits) * sizeof(unsigned long));
    int excess = capacity - num_bits;
    if (excess > 0)
    {
        unsigned long mask;
...
bool operator==(const BitArray& a, const BitArray& b)
{
    ...
    int words = a.get_words_needed(a.size());
    for (int i = 0; i < words; ++i)
    {
